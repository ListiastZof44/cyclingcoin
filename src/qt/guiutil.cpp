...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("cyclingcoin"))
        return false;
...
{
    // Convert cyclingcoin:// to cyclingcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("cyclingcoin://"))
    {
        uri.replace(0, 12, "cyclingcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CYC.lnk";
}
...
{
    return GetAutostartDir() / "cyclingcoin.desktop";
}

bool GetStartOnSystemStartup()
{
    boost::filesystem::ifstream cyclingcoinFile(GetAutostartFilePath());
    if (!cyclingcoinFile.good())
        return false;
    // Scan through file for "Hidden=true":
    std::string line;
    while (!cyclingcoinFile.eof())
    {
        getline(cyclingcoinFile, line);
        if (line.find("Hidden") != std::string::npos &&
            line.find("true") != std::string::npos)
            return false;
    }
    cyclingcoinFile.close();
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        boost::filesystem::ofstream cyclingcoinFile(GetAutostartFilePath(), std::ios_base::out|std::ios_base::trunc);
        if (!cyclingcoinFile.good())
            return false;
        // Write a bitcoin.desktop file to the autostart directory:
        cyclingcoinFile << "[Desktop Entry]\n";
        cyclingcoinFile << "Type=Application\n";
        cyclingcoinFile << "Name=CYC\n";
        cyclingcoinFile << "Exec=" << pszExePath << " -min\n";
        cyclingcoinFile << "Terminal=false\n";
        cyclingcoinFile << "Hidden=false\n";
        cyclingcoinFile.close();
    }
    return true;
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CYC-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  cyclingcoin-qt [" + tr("command-line options") + "]                     " + "\n";
