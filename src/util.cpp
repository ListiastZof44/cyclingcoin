...
#else
    const char* pszModule = "cyclingcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CYC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CYC
    // Mac: ~/Library/Application Support/CYC
    // Unix: ~/.CYC
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CYC";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "CYC.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cyclingcoind.pid"));
